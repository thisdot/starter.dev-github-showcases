...
const matchText = (target: string, value: string) => target?.match(new RegExp(value, 'i'));
...
export const repoDataFilteredByLanguage = (repos: Repo[], language: string): Repo[] => {
  ...
  if (repos && language && language !== defaultLanguage) {
    response = repos.filter((repo: Repo) => matchText(repo?.primaryLanguage?.name, language));
