...
export const PullRequestIssueTab = component$(
  ({ openCount, closedCount, tabType, milestonesOption, labelOption, sortOption }: PullRequestIssueTabParams) => {
    ...
    return (
      <div class="flex flex-wrap space-x-1 space-y-2 md:space-x-0 md:space-y-0 items-center justify-between p-4 bg-gray-100 border-b">
        <div class="flex space-x-4">
          <button class={openBtnClasses} onClick$={() => (tab.activeTab = TABS.OPEN)}>
            {getIcon()}
            <span>{openCount}</span>
            Open
          </button>
          <button class={closedBtnClasses} onClick$={() => (tab.activeTab = TABS.CLOSED)}>
