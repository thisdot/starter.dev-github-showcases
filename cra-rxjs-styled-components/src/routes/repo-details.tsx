...
export default function RepoDetails() {
  const [repoDetails, setRepoDetails] = useState<RepositoryDetails>({ repo: null });
  const [loading, setLoading] = useState(true);
  const params = useParams();
  const { repo, openPr, closedPr, rootFileInfo, issues } = repoDetails;

  const request = (url: string) => fromFetchWithAuth(url, {
  ...
  useEffect(() => {
    forkJoin([
      ...
      request(`${SINGLE_USER_REPO(params.username!, params.repo!)}/pulls?state=closed`),
