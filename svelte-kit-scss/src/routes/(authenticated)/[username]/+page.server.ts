...
export const load: PageServerLoad = async ({fetch, params}) => {
  ...
  const [userInfo, userOrgs] = await Promise.all([
    fetch(fetchUserInfoUrl.toString()).then(response => response.json() as Promise<UserApiResponse>),
    fetch(`https://api.github.com/user/orgs`).then(response => response.json() as Promise<UserOrgsApiResponse>),
  ])
  return {
